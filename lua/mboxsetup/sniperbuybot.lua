--region == Credits & Info ==
-- SniperBuyBot
-- Forked (almost all stolen) from VacciBucks
-- Intends for the user to be equipped with the Hitman's Heatmaker.
-- No force check for this, only because other weapons are technically supported.
...
local UPGRADE_DELAY = 0.2 -- Increased delay from 0.05 to reduce necessary re-runs.
local SEQUENCE_END_COOLDOWN = 1.0
local nextUpgradeTime = 0
local sequenceEndTime = 0
local upgradeQueue = {}
local isExploiting = false
local hasSequenceRun = 0 -- Limited to 3 runs (elsewhere), if this fails then the bot should be investigated.
...
--region == Server Change Detection ==
...
callbacks.Register("CreateMove", function(cmd)
    -- Check for the flag set by Helper
    if EnableBuyBot then
        -- Enable VacciBucks functionality
        autoWalkEnabled = true
        shouldGuidePlayer = true
        AddNotification("EnableBuyBot Auto Walk enabled by MultiboxHelper!", "success")

        -- Reset the flag after enabling
        EnableBuyBot = false
    end
    CheckServerChange()
...
local function ProcessUpgradeQueue()
    ...
    if #upgradeQueue == 0 then
        if isExploiting then
            ...
            AddNotification("Reset with [K] to continue upgrades.", "success")
            isExploiting = false
            shouldGuidePlayer = false
...
local function TriggerUpgradeSequence()
    local currentTime = globals.CurTime()
    ...
    if hasSequenceRun >= 3 then
        return
    end

    lastExploitTime = currentTime
    isExploiting = true
    hasSequenceRun = hasSequenceRun + 1
    ...
    local function SetUpgradeQueue()
        local upgradeQueueGhost = {
            ...
            -- 25% Damage
            { type = "begin" },
            { type = "upgrade", slot = 0,  id = 0,  count = 1 },
            { type = "upgrade", slot = 0,  id = 0,  count = 1 },
            { type = "upgrade", slot = 0,  id = 0,  count = 1 },
            { type = "upgrade", slot = 0,  id = 0,  count = 1 },
            { type = "end",     count = 4 },
            ...
            -- Explosive Headshot
            { type = "begin" },
            { type = "upgrade", slot = 0,  id = 40, count = 1 },
            { type = "upgrade", slot = 0,  id = 40, count = 1 },
            { type = "upgrade", slot = 0,  id = 40, count = 1 },
            { type = "end",     count = 3 },
            -- +20% Reload Speed
            { type = "begin" },
            { type = "upgrade", slot = 0,  id = 35, count = 1 },
            { type = "upgrade", slot = 0,  id = 35, count = 1 },
            { type = "upgrade", slot = 0,  id = 35, count = 1 },
            { type = "end",     count = 3 },
            -- 25% Faster Charge
            { type = "begin" },
            { type = "upgrade", slot = 0,  id = 17, count = 1 },
            { type = "upgrade", slot = 0,  id = 17, count = 1 },
            { type = "upgrade", slot = 0,  id = 17, count = 1 },
            { type = "upgrade", slot = 0,  id = 17, count = 1 },
            { type = "end",     count = 4 },
            ...
            { type = "begin" },
            { type = "upgrade", slot = 0,  id = 12, count = 1 },
            { type = "end",     count = 1 },
            -- +50% Ammo Capacity
            { type = "begin" },
            { type = "upgrade", slot = 0,  id = 6,  count = 1 },
            { type = "upgrade", slot = 0,  id = 6,  count = 1 },
            { type = "upgrade", slot = 0,  id = 6,  count = 1 },
            { type = "upgrade", slot = 0,  id = 6,  count = 1 },
            { type = "upgrade", slot = 0,  id = 6,  count = 1 },
            ...
            -- +25 Health on Kill
            { type = "begin" },
            { type = "upgrade", slot = 0,  id = 11, count = 1 },
            { type = "upgrade", slot = 0,  id = 11, count = 1 },
            { type = "upgrade", slot = 0,  id = 11, count = 1 },
            { type = "upgrade", slot = 0,  id = 11, count = 1 },
            { type = "end",     count = 4 },
            ...
            -- +25% Blast
            { type = "begin" },
            { type = "upgrade", slot = -1, id = 56, count = 1 },
            { type = "upgrade", slot = -1, id = 56, count = 1 },
            { type = "upgrade", slot = -1, id = 56, count = 1 },
            { type = "end",     count = 3 },
            -- +25% Bullet
            { type = "begin" },
            { type = "upgrade", slot = -1, id = 57, count = 1 },
            { type = "upgrade", slot = -1, id = 57, count = 1 },
            { type = "upgrade", slot = -1, id = 57, count = 1 },
            { type = "end",     count = 3 },
            -- +30% Crit
            { type = "begin" },
            { type = "upgrade", slot = -1, id = 58, count = 1 },
            { type = "upgrade", slot = -1, id = 58, count = 1 },
            { type = "upgrade", slot = -1, id = 58, count = 1 },
            { type = "end",     count = 3 }
        }
...
--region == Draw UI & Main Logic ==
callbacks.Register("Draw", function()
    local offsetX, offsetY = 10, 10
    local paddingX, paddingY = 10, 5
    local baseText = "Sniper Buybot"
...
callbacks.Register("CreateMove", function(cmd)
    ...
    if autoWalkEnabled then
        ...
        -- upd guidance states
        if not inZone and not isExploiting then
            local newMidpoint = FindUpgradeStations(me)
            if newMidpoint then
                midpoint = newMidpoint
                if not shouldGuidePlayer then
                    shouldGuidePlayer = true
                    AddNotification(
                        "Found upgrade station at: " ..
                        tostring(midpoint.x) .. ", " .. tostring(midpoint.y) .. ", " .. tostring(midpoint.z), "info")
                end
            else
    ...
    if me:GetPropInt('m_bInUpgradeZone') == 1 and not isExploiting then
        TriggerUpgradeSequence()
    end

    if isExploiting and me:GetPropInt('m_bInUpgradeZone') ~= 1 then
        ForceCleanup()
        AddNotification("Sequence cancelled - left zone!", "error")
        return
    end

    if isExploiting and not me:IsAlive() then
        ForceCleanup()
...
--region == Integration with MultiboxHelper ==
function StartSSBuybot()
    EnableBuyBot = true
end

function StopSSBuybot()
    ForceCleanup()
end

--endregion

--region == Cleanup on Unload ==
callbacks.Register("Unload", function()
    ForceCleanup()
end)
--endregion

AddNotification("Buybot loaded! [K] for cleanup, [L] for auto walk", "info")
